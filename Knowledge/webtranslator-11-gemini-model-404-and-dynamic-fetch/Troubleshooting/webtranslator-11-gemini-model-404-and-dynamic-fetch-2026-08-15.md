...
* **[구형 하드코딩 모델의 한계]:** 초기 `gemini-2.5-flash` 모델은 구형으로 무료 허용량(RPD 20 수준)이 매우 적었고, 서비스 종료로 404 Not Found가 발생함. [FACT]
...
* **[OpenAI 유료 단가 구조와 자동 선택의 과금 위험]:**
  * OpenAI는 무료 플랜이 없으며, 모델별 1M 토큰당 가격(Input/Output)이 큰 차이를 보임 (GPT-5.6 Sol: $5/$30, GPT-5.6 Terra: $2/$12, GPT-5.6 Luna: $0.20/$1.20, GPT-4o: $2.50/$10, GPT-4o mini: $0.15/$0.60).
  * AI 자동 선택기가 초저가 모델 대신 비싼 상위 모델(`gpt-4o`)을 임의 호출하여 선결제 $10 크레딧이 조기 소진되는 문제 발생. [FACT]
* **[자동 선택 폐기 및 사용자 직접 모델 고정]:** 런타임 자동 선택기를 전면 폐기하고, 사용자가 초가성비 모델(`gemini-flash-lite-latest`, `gpt-5.6-luna`/`gpt-4o-mini`)을 직접 지정하도록 전환. [FACT]
...
2. OpenAI: 초저가 모델(`gpt-5.6-luna` / `gpt-4o-mini`) 설정으로 토큰 단가 최소화.
...
* [FACT] Gemini의 경우 Pro $\rightarrow$ Flash $\rightarrow$ Flash-Lite 순으로 일일 요청 수(RPD) 한도가 비약적으로 증가하므로 대량 번역에는 Flash-Lite 계열을 권장한다.

---

#### 📋 Feedback History
* **2026-08-19:** Gemini 쿼터 3대 축(RPM/TPM/RPD), GPT 공식 단가표, 헬퍼 인스펙터 발전 방향 반영 완료 (Status: Verified).
