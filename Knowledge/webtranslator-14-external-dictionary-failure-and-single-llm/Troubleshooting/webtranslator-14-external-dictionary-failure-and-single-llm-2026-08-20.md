...
* **[단어/문장 팝업 UI 통합 및 렌더링 최적화 (`dictionaryPopup.js`)]:**
  * 드래그 대상에 따라 단어 모드와 문장 모드로 분리하려던 복잡한 UI 분기 및 다중 모달 관리를 배제하고, 단일 플로팅 카드 컴포넌트로 통합. [FACT]
  * **스키마 기반 동적 렌더링:** LLM JSON 응답의 `definitions` 구조(`pos`, `example` null 여부)에 따라 단어 카드(발음/품사/뜻/예문)와 문장 번역 카드(직접 번역문)를 동일 컨테이너 내에서 조건부 렌더링. [FACT]
  * **뷰포트 적응형 위치 계산:** `Selection.getRangeAt(0).getBoundingClientRect()` 기반 텍스트 상단 배치, 상단 공간 부족 시 하단 반전(Flip), 좌우 경계 벗어남 방지 오프셋 클램핑(Clamping) 적용. [FACT]
  * **단일 인스턴스 라이프사이클 관리:** 연속 드래그 시 팝업 인스턴스를 재사용하여 불필요한 DOM 재생성을 방지하고, 외부 클릭/`ESC` 키 바인딩 정리로 메모리 누수 원천 차단. [FACT]
...
3. **단일 팝업 UI 통합 및 최적화 (`dictionaryPopup.js`):** [★★★★★] ✅ Verified 2026-08-20
   - 스키마 기반 조건부 뷰 전환, 뷰포트 적응형 스마트 위치 계산(상하 반전/좌우 클램핑), 단일 인스턴스 재사용을 통한 DOM 최적화 적용.
...
* **피드백 내용:**
  1. 서론 문단 분리 및 6편 선택 영역 번역(드래그 번역) 기능과의 연계/UI·UX 강화 배경 문장 정제 반영.
  2. 외부 무료 API의 Rate Limit 한계를 과거 Google Translate 연동 경험(무료 티어 LLM 대비 사전 호출 빈도의 중요성)과 비교하여 설명 보강.
  3. `dictionaryPopup.js` 단일 플로팅 카드 UI 아키텍처 상세 보강: 스키마 기반 동적 뷰 렌더링, 뷰포트 적응형 위치 계산(Flip/Clamping), 단일 인스턴스 라이프사이클 관리.
  4. 프롬프트 코드 블록의 문자열 연결 연산자(`+`) 제거 및 템플릿 리터럴 형태로 클린업.
  5. 지식 베이스 및 블로그 HTML 게시글 양방향 동기화.
