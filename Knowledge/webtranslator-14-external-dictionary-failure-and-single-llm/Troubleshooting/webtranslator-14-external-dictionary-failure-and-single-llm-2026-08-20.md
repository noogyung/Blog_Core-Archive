...
* **[Kaikki 전체 덤프 데이터의 브라우저 확장 프로그램 직접 로드 불가 결함]**
  * **증상:** 수십 기가바이트에 달하는 사전 JSON 파일로 인해 브라우저 로컬 스토리지 및 메모리 한계 초과.
  * **원인:** Wiktionary 기반 원천 데이터의 방대한 용량과 클라이언트 사이드 인덱싱 부재.
  * **해결법:** 자체 서버 구축 대신 경량 LLM(Gemini Flash / GPT-4o-mini)에 정밀한 단일 프롬프트를 전송하여 0.8초 만에 필요한 데이터만 스트리밍 수신. [FACT]
  * **신뢰도:** [★★★★★]
* **[서드파티 무료 사전 API 연동 시 다국어 부실 및 차단 리스크]**
  * **증상:** 비영어권(CJK) 단어 검색 시 결과 누락 및 단기간 반복 호출 시 IP 차단 발생 우려.
  * **원인:** 무료 사전 API 서비스의 호출 한도(Rate Limit) 제약 및 비영어권 데이터베이스 미지원.
  * **해결법:** 외부 사전 의존성을 전면 배제하고 사용자 개인 API Key를 사용하는 LLM 파이프라인으로 전환하여 안정성 확보. [FACT]
  * **신뢰도:** [★★★★★]
