...
* **[사건 3: "5번 스킬만 왜 이래?" 뒤늦게 추가된 멀티 블로그 기능으로 인한 네이밍 파편화]**
  * **비하인드 스토리:** 모듈화 초기에는 단일 블로그만 염두에 두고 1~4번 스킬(`주제`, `피드백`, `게시글`, `발행완료`)만 구축하여 사용하고 있었다. 그러다 멀티 블로그 운영 필요성이 생기며 `5_블로그추가` 스킬을 뒤늦게 신설했는데, 1~4번은 폴더명에 번호가 없고 5번만 번호가 붙은 채 Frontmatter `name`과 어긋나면서 슬래시 커맨드 자동완성 및 도구 호출 시 혼선이 생겼다.
  * **근본 원인:** 기능 확장 시점마다 네이밍 규칙을 일관되게 정돈하지 않고 방치했던 구조적 부채였다.
  * **→ 변화:** 디렉토리명 전체를 `1_주제/`, `2_피드백/`, `3_게시글/`, `4_발행완료/`, `5_블로그추가/`로 100% 일원화하여 디렉토리와 슬래시 커맨드가 1:1로 완벽히 매칭되도록 정규화했다. [USER VERIFIED]

* **[사건 4: 정상 동작도 안 하는 '구상' 데이터와 AI의 'Next Slug 뇌피셜'이 부른 전면 수정 대참사]**
  * **비하인드 스토리:** 2편까지의 시리즈 네비게이션은 실제 코드로 정상 작동하는 상태가 아니라 머릿속으로만 '이렇게 만들자'고 메모해둔 구상 단계에 불과했다. 그런데 AI가 이 미완성 구상 데이터를 마치 구현된 표준인 양 읽고, 존재하지도 않는 다음 편 링크(`Next Slug`)를 자기 멋대로 상상(Hallucination)하여 작성하기 시작했다. 그 결과 발행된 모든 글의 시리즈 링크가 404 에러로 터져나갔고, 결국 과거에 발행했던 모든 게시글의 HTML을 일일이 열어 뜯어고쳐야 하는 끔찍한 수작업 노가다가 발생했다.
  * **근본 원인:** 동작 검증이 안 된 '구상'을 문서에 방치한 점, 그리고 AI에게 미래 시점의 데이터(`Next Slug`, `series_total`)를 예측하도록 허용한 치명적 설계 결함이었다.
...
* **[사건 5: Control Plane vs Data Plane 분리 작업 중 테마 파일 초기화 참사와 롤백 재작업]**
  * **비하인드 스토리:** BlogDocs 단일 레포지토리에서 데이터와 지능을 분리하기 위해 Control Plane(`BlogDocs`)과 Data Plane(`Blog_Core-Archive`)으로 레포지토리를 물리적으로 쪼개고 디렉토리 구조를 재편하던 중, 기껏 피땀 흘려 커스터마이징해둔 Median UI 테마 XML 파일과 Blogger 레이아웃 위젯 설정이 통째로 덮어씌워지거나 초기화되는 대형 사고가 터졌다. 수많은 CSS 클래스 수정본과 사이드바 링크 위젯, 상단 슬라이더 공지 위젯(`slider-notice-widget`) 설정이 순식간에 백지가 되었고, 백업본과 기억을 더듬어가며 새벽 내내 테마 코드를 한 줄 한 줄 다시 복원하고 재작업해야 했다.
  * **근본 원인:** 분산 아키텍처 마이그레이션 과정에서 테마 및 정적 설정 파일에 대한 명확한 책임 분리와 형상 관리 절차가 부재했던 탓이었다.
  * **→ 변화:** 테마 커스텀 XML(`_theme-median-ui-...-Custom.xml`)을 Data Plane에 확실히 버전 관리하고, `/5_블로그추가` 스킬 내에 테마 적용 및 정적 페이지(About, Privacy, Contact, Terms) 4종 생성 로직을 테마 중립적으로 완벽히 내재화하여 다시는 테마 세팅이 유실되지 않도록 방어선을 구축했다. [USER VERIFIED]
...
* [OPINION] 이번 v3.0 구축 과정은 말 그대로 **"참사가 낳은 가드레일의 역사"**였다. 자연어 폭주를 겪고 슬래시 커맨드를 만들었고, 뚱뚱한 룰의 토큰 낭비를 겪고 스킬 분리를 단행했으며, AI의 Next Slug 상상 참사를 겪고 동적 피드 탭 네비게이션을 발명했고, 테마 초기화 참사를 겪고 레포지토리 형상관리를 완성했다. AI 자동화 시스템의 신뢰성은 이론이 아니라 수많은 오류를 직접 들이받으며 쌓아 올린 가드레일의 두께에서 나온다. — 출처: 사용자 실전 경험 [USER VERIFIED]
