...
* [OPINION] Blogger의 기본 댓글 체계를 버리고 Vercel + Neon + Cloudflare Turnstile 기반의 Waline으로 전환한 것은 방문자의 진입 장벽(익명/게스트 허용 및 이메일 입력)을 낮추면서도 무인 봇 스팸을 효과적으로 방어하고, 마크다운과 이미지 첨부까지 지원하는 최적의 무료 서버리스 솔루션임을 확인했다.
...
* [OPINION] 향후 계획: 지금까지 축적한 포스팅 자동화 경험을 여러 블로그 운영자에게 도움이 되는 범용 패키지로 제품화하고, 유료 테마에 종속된 'Core-Archive 전용 설정'과 '공용 코어 엔진'을 명확히 분리하여 무료 기본 테마나 새로운 테마에서도 쉽게 시작할 수 있는 멀티 블로그 환경으로 확장할 구상임.
