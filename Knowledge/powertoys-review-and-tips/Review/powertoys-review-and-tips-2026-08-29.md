...
3. **특수 환경 (메인 PC가 Wi-Fi 내부망 + 랜선 외부망으로 분리된 경우):**
   ...
   - *(선택 / 경로 확실화)* 네트워크 연결 속성(`ncpa.cpl`) > `IPv4` > `고급`에서 자동 메트릭 해제 후 수동 지정:
     - 유선 랜(외부망): 인터페이스 메트릭 `10`
     - Wi-Fi(내부망): 인터페이스 메트릭 `20`
     - *설정 설명 및 이유:* 인터페이스 메트릭은 어댑터 간 라우팅 우선순위를 정하는 값(숫자가 작을수록 우선순위가 높음)입니다. 일반적인 상황에서는 Windows의 자동 메트릭으로도 처리되어 필수로 설정할 필요는 없으나, 듀얼 어댑터 환경에서 기본 인터넷 트래픽이 외부망 유선 랜으로 확실하게 나가도록 보장하고 어댑터 간 라우팅 혼선을 방지하기 위해 설정한 것입니다.
...
* **[Mouse Without Borders 다중 네트워크 어댑터 분리 환경에서의 연결 실패]** *(사용자 검증 추가 — 2026-08-30)*
  ...
  * 해결법:
    ...
    3. 필요 시 네트워크 어댑터 속성(`ncpa.cpl`)의 IPv4 고급 설정에서 인터페이스 메트릭(유선 랜 10, Wi-Fi 20)을 수동 지정하여 인터넷 기본 경로와 내부망 통신 경로를 확실히 분기. (기본적으로 필수는 아니나 라우팅 우선순위를 확실히 보장하기 위한 조치) [USER VERIFIED]
  * 신뢰도: [★★★★★]
...
* [OPINION] **Mouse Without Borders 장단점 및 네트워크 메트릭:**
  ...
  * 네트워크 팁: 듀얼 네트워크 어댑터 사용 시 수동 메트릭 설정은 필수는 아니지만, 외부망 유선 랜(메트릭 10)과 내부망 Wi-Fi(메트릭 20)를 지정해두면 인터넷 경로 혼선 없이 확실하게 통신 경로를 분리할 수 있음.
...
* **수정 및 반영 사항:**
  ...
  - IPv4 인터페이스 메트릭 옵션의 개념(라우팅 우선순위 가중치) 및 설정 이유(인터넷 경로 확실화) 상세 서술 추가
