...
* **[텍스트 검출(Detection) 파편화와 인식(Recognition) 붕괴]:**
  * 웹 환경의 다양한 폰트와 이미지 해상도에 대해 Detection 모델이 문장/단어 단위가 아닌 글자·획 단위로 과도하게 잘게 쪼개진 바운딩 박스를 대량 생성함. [FACT]
  * 잘게 쪼개진 크롭 이미지가 Recognition 모델에 전달되었으나, 텐서 전처리(정규화 및 회전 보정) 불일치로 인해 글자 인식률이 0%에 수렴하며 콘솔에 `[WT PaddleOCR] 텍스트 없음` 에러만 도배되는 완전한 기능 실패가 발생함. [FACT]
...
1. **ONNX Runtime Web 및 PaddleOCR 모듈 초기화 구성 시도:** [★★★★★]
...
* **[글자 단위 박스 난잡화 및 Recognition 인식률 0% 에러 도배 (`[WT PaddleOCR] 텍스트 없음`)]**
  * 증상: 바운딩 박스가 무의미하게 미세 단위로 쪼개져 렌더링되고, 콘솔에 `[WT PaddleOCR] 텍스트 없음` 에러만 연속 발생.
  * 원인: 웹 환경의 다양한 비정형 폰트/해상도에 대한 Detection 임계값 미세조정 실패 및 크롭 영역 전처리 텐서 정규화 불일치.
  * 해결법: 실사용 수준의 품질 달성이 불가능함을 판단하고, 환각 보고서를 폐기한 뒤 이미지 번역 파이프라인 개발을 전면 중단. [FACT]
  * 신뢰도: [★★★★★]
...
* [FACT] 브라우저 내에서 무거운 온디바이스 딥러닝 모델(OCR 등)을 구동할 때는 WebAssembly 힙 메모리 누착 방지를 위해 메인 스레드가 아닌 생명주기 제어가 가능한 Web Worker나 WebGPU 환경이 필수적이다.
...
* [ ] PaddleOCR의 탭 램 점유율(500MB+)과 초기화 지연(8~10초) 수치가 실제 테스트 환경에서 기록된 수치와 일치하는지 확인
* [ ] `modules/image_translator/`로 격리된 코드 외에 기존 `src/` 내 텍스트 번역 코드에 잔여 의존성이 완전히 제거되었는지 확인
* [ ] v1.0.0 출시 버전에서 이미지 번역 제외 후 텍스트 번역 파이프라인이 정상 동작하는지 확인
